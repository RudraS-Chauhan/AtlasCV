...
const ProjectTile: React.FC<{
    ...
    <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 relative space-y-4 animate-in fade-in slide-in-from-bottom-2">
        <button type="button" onClick={() => onRemove(project.id)} className="absolute top-4 right-4 text-slate-400 hover:text-red-500 transition-colors">✕</button>
        <div className="flex items-center gap-2 mb-2">
             <div className="w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-xs">{index + 1}</div>
             <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300">Project Details</h4>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <input type="text" placeholder="Project Name" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm" value={project.name} onChange={(e) => onChange(project.id, 'name', e.target.value)} />
             <input type="text" placeholder="Link (Github/Live)" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm" value={project.link} onChange={(e) => onChange(project.id, 'link', e.target.value)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <input type="text" placeholder="Start Date (e.g. Jan 2024)" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm" value={project.startDate} onChange={(e) => onChange(project.id, 'startDate', e.target.value)} />
             <input type="text" placeholder="End Date (e.g. Mar 2024)" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm" value={project.endDate} onChange={(e) => onChange(project.id, 'endDate', e.target.value)} />
        </div>
        <textarea placeholder="2-5 lines on impact, tech stack, and results..." rows={3} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm resize-none" value={project.description} onChange={(e) => onChange(project.id, 'description', e.target.value)} />
    </div>
);
          ...
          {currentStep === 4 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-8">
                  <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
                     ...
                     <TextareaField id="skills" label="Your Skills Arsenal *" placeholder="Languages: Java, Python, JavaScript&#10;Frameworks: React, Node.js&#10;Tools: Git, AWS, Docker" value={formData.skills} onChange={handleChange} required rows={6} helpText="List everything you're good at. We'll categorize them." autoFocus />
                  </div>
              </div>
          )}
